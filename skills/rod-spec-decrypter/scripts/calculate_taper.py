...
def run_diagnostics(tip, butt, length, max_lure):
    ...
    if butt < SLIM_BUTT_FLAG:
        ...
        print("        高錐度比在此被**極細的竿尖**灌高（分母太小），不是因為元端特別粗。")
        print("        → **級距高估**：EX-Fast 這一級不可信，實際多半只到 Fast。以原廠標的調性為準。")
        print("        → 真正的意思是**絕對剛度低**——很輕的負載就會彎得很深。")
        print("")
        print("        🔴 **但「軟」不等於「彎曲點靠後」，這兩件事必須分開講。**")
        print("            本警示**不得**用來推論「整支一起彎」或「彎曲點後移」。")
        print("            彎曲點的位置由錐度分佈決定，而極細的竿尖正是把彎曲**往前拉**。")
        print("            一支竿可以同時是「很軟」與「彎曲點很靠前」——深彎與前調並不矛盾。")
        # 校準樣本為 702UL+FS-ST23；型號刻意不印出，理由同 print_band_caution()。
        print("        → 已知案例：元徑 7.4mm、Ratio 10.57 判 EX-Fast，**官方標 F**（不是 R）。")
        print("            官方標 F 本身就證明彎曲點在前——所以此處**只推翻 EX-，不推翻 Fast**。")
    else:
