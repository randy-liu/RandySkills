...
4. **嚴格動態載入廠牌技術字典**：分析特定廠牌的釣竿時，**必須**透過 `view_file` 讀取同目錄 `references/` 底下對應的字典檔（如 `daiwa_technology.md`）。在解析技術名詞時，必須採取「精準比對 (Exact Match)」。若使用者提供或搜尋到的技術名詞**不存在**於該字典檔中，請直接在報告中揭露「該項技術目前不在 Skill 字典庫中，需要更新 Skill」，**絕對不要擅自臆測或腦補**該技術的物理影響。
