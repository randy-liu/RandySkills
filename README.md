...
- **雙模產圖**：`plot-zenaq`（45 度持竿的等比例形狀圖）與 `plot-engineering`（水平持竿的分析圖，四層：竿身尺寸帶／額定等比例／額定放大／深彎等比例）。兩者**共用同一組負載與同一套物理**，差別只在持竿角度。
  🔴 **額定區給兩格是刻意的**：放大那格能讀「沉幾公分」，但它的**形狀是假的**（實際只彎 2〜4%，看起來像 25%）。所以另給一格**精確 1:1**，形狀歸形狀、數字歸數字，標題各自寫明。
  ⚠️ 1:1 是由面板實際英吋反推 ylim 算出來的（不是 `set_aspect`），並**從存檔 PNG 量 px/cm 驗證**，不只信程式自己的宣稱。最上層另有一條竿身尺寸帶，標出握把與可利用竿長。
