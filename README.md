...
- **雙模產圖**：`plot-zenaq`（45 度持竿的等比例形狀圖）與 `plot-engineering`（水平持竿的分析圖，上下兩面板分開畫「額定區間」與「搏魚」）。兩者**共用同一組負載與同一套物理**，差別只在持竿角度。
- **負載跟著竿子走**：額定區依該竿的路亞負載推演；搏魚點由**適合ライン**推導（`上限 lb × 453.6 × 1/3`），不是固定克數——竿子承受的持續拉力上限由洩力決定，不由魚的大小決定。
- **跨竿對比圖用「各自基準」**：`shape`（每支都彎到竿尖下沉全長 1/3，深度統一後純比形狀）與 `fight`（各自的洩力上限）。固定絕對負載對混著 L 到 H 的一組竿並不公平，而且結果只是在重述力量字母。
...
> 而洩力不可能高過線的破斷強度，慣例設在 1/3 → `搏魚負載 = 適合ライン上限(lb) × 453.6 × 1/3`。
> 🟡 1/3 是釣魚圈慣例，非原廠規格。細節見 `rod-curve-generator` 的 README。
